...
class SOLUTION:
	...
	def __init__(self, myID, showArms):
		...
		self.myID = myID
		if showArms:
			self.weights = numpy.random.rand(c.numSensorNeurons, c.numMotorNeuronsWithArms)
		else:
			self.weights = numpy.random.rand(c.numSensorNeurons, c.numMotorNeuronsWithoutArms)			
